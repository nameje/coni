...
이 모든 '사고'와 '추론'의 과정은 AI의 머릿속이 아닌, **파일 시스템 위에 명시적으로 기록되는 '실행의 사슬(Chain of Execution)'** 로 구체화됩니다. 오케스트레이터라는 '침묵의 지휘자'의 통제 아래, 플래너의 '설계'와 익스큐터의 '실행'은 모두 추적 가능한 파일로 남습니다.
...
- **오케스트레이터 (Orchestrator):** 시스템의 **침묵하는 지휘자(Silent Conductor)**. 파일 시스템의 상태 변화에 따라, 플래너, 익스큐터, Communicator에게 명확한 명령을 내리는 유일한 내부 프로세스 관리자.
    
- **플래너 (Planner):** 시스템의 **지능적 항해사(Intelligent Navigator)**. 오케스트레이터의 명령에 따라, 과거의 모든 실행 이력과 현재의 자산을 참조하여 최적의 경로(계획)를 설계. 이는 **상위 수준에서는 '전략적 자산 취득 계획'을, 하위 수준에서는 '구체적 실행 방법론 선택'을 포함**합니다.
    
- **익스큐터 (Executor):** 시스템의 **충실한 실행자(Faithful Executor)**. 오케스트레이터의 위임에 따라, 플래너가 설계한 모든 종류의 Task를 정확하게 수행하고, 이때 자신의 판단을 최소화하고, task_purpose에 명시된 방법론과 절차를 기계적으로 따르는 것을 원칙으로 합니다. 필요시 '특수 목적 에이전트'에게 하위 임무를 위임.
...
- **guidelines/ (가이드라인 폴더):** 최종 결과물의 **'형식(Format)', '스타일(Style)', '템플릿(Template)'** 등을 규정하는 지침 파일들이 위치하는 입력 소스. 이는 추상적인 규칙이 아니라, Task에 직접 입력될 수 있는 물리적인 파일임.
...
- **3차원 실행 공간 (The 3D Execution Space):** 모든 **단일 run**은 아래 3개의 차원으로 구성된 독립적인 '실행 공간'입니다.
    ...
    3. **3차원 (확장성 - Expansion):** 핵심 Task에서 하위 에이전트에게 임무를 위임하는 재귀적 복잡성의 깊이.
...
- **루프 조건:** 현재 current_stage_id에 속하는 PENDING 상태의 Task가 더 이상 없을 때까지 아래 과정을 반복합니다.
    ...
    1. **Task 선택:** tasks.md에서 현재 stage_id에 속하는, execution_order가 가장 낮은 PENDING Task를 찾습니다. 만약 없다면, 이 Task 실행 루프를 종료하고 **(d) Stage 완료 처리**로 이동합니다.
        
    2. **진행 과정 기록:** db/process_runs.md의 current_task_id를 방금 찾은 Task의 ID로 업데이트합니다.
        
    3. **'Task 패키지' 실행 위임:** 해당 task_id를 **익스큐터에게 위임**하여, **2.2.3. Task 패키지 실행 및 하위 임무 위임** 프로토콜에 따라 선행/핵심/후행 작업을 순차적으로 처리하도록 지시합니다.
        
    4. **결과 처리:** 익스큐터로부터 성공 또는 실패 보고를 받습니다.
        
        - **성공 시:** **2.2.3 프로토콜**에 따라, tasks.md와 tool_tasks.md의 status가 COMPLETED로 변경되고, 생성된 모든 결과물 정보가 **전역 db/knowledge_base_catalog.md에 등재**됩니다.
            
        - **실패 시:** **2.2.4. 오류 처리** 프로토콜로 즉시 이동합니다.
...
---
### **2.2.3. Task 패키지 실행 및 하위 임무 위임**

- **목표:** tasks.md에 정의된 단일 핵심 임무를, 필요에 따라 전문화된 **하위 에이전트(Specialized Agents)** 에게 보조 임무를 위임하며, 하나의 완전한 '패키지'로서 성공적으로 완수하고, 그 모든 결과를 시스템의 전역 지식 베이스에 통합하는 것.
    
- **트리거:** 오케스트레이터의 핵심 제어 루프(2.2.2.3.c)가 실행할 PENDING 상태의 Task를 선택하고, 그 task_id를 익스큐터에게 전달하여 본 프로토콜을 시작하는 시점.
    
- **주요 행위자:**
    
    - **오케스트레이터:** 전체 'Task 패키지'의 실행 순서를 지휘하고, 최상위 상태를 관리.
        
    - **익스큐터:** 오케스트레이터로부터 최상위 임무를 위임받아, 단순 기능은 직접 수행하고, 복합적인 기능은 적절한 **하위 에이전트에게 재위임**하는 **'중간 관리자(Middle Manager)'** 역할.
        
    - **특수 목적 에이전트 (하위 에이전트):** 익스큐터로부터 위임받은 특정 임무(웹 검색, 시각화 등)를 자율적으로 수행하고 결과를 보고.
...
오케스트레이터는 위임한 task_id에 대해 아래의 절차를 순차적으로, 예외 없이 수행합니다.

**1. [준비] Task 명세 로드 (Load Task Specification)**

- **오케스트레이터의 행동:**
    
    1. runs/{run_id}/db/tasks.md 파일을 읽어, 현재 실행 중인 task_id에 해당하는 행의 모든 정보를 확보합니다.
        
    2. pre_tool_purpose와 post_tool_purpose 컬럼의 존재 여부를 확인하여, 이번 패키지에 하위 임무 위임이 포함되는지 파악합니다.
        

**2. [1단계] 선행 하위 임무 위임 (Pre-Mission Delegation)**

- **트리거:** 로드된 Task 명세에 pre_tool_purpose가 존재하는 경우. 만약 존재하지 않으면 이 단계는 건너뜁니다.
    
- **오케스트레이터의 행동:**
    
    1. **하위 임무 식별:** runs/{run_id}/db/tool_tasks.md에서 parent_task_id가 현재 task_id와 일치하며 timing이 PRE인 모든 하위 임무들을 execution_order 순으로 정렬합니다.
        
    2. **하위 임무 순차 위임 (Loop):** 정렬된 목록의 각 tool_task_id에 대해 아래를 반복합니다.  
        a. **실행 위임:** 해당 tool_task_id를 **익스큐터에게 위임**합니다.  
        b. **익스큐터의 행동:**  
        i.  tool_tasks.md에서 위임받은 tool_task_id의 tool_type과 tool_task_purpose를 확인합니다.  
        ii. tool_type에 해당하는 **'특수 목적 에이전트'(예: Web Search Agent)를 호출**하고, tool_task_purpose를 새로운 '임무 요약서(Mission Brief)'로 전달하여 임무를 재위임합니다.  
        iii. 하위 에이전트가 임무를 완수하고 결과물(예: 요약 보고서 파일)을 반환할 때까지 대기합니다.  
        iv. 최종 결과물 파일 경로와 성공 신호를 오케스트레이터에게 보고합니다.  
        c. **오케스트레이터의 결과 처리:**  
        i. 보고받은 결과물 정보를 **전역 db/knowledge_base_catalog.md에 등재**합니다.  
        ii. tool_tasks.md에서 해당 tool_task_id의 status를 COMPLETED로 변경합니다.
        
    
    - **[아키텍처 원칙: 재귀적 위임(Recursive Delegation)]** 오케스트레이터가 익스큐터에게 위임하고, 익스큐터가 다시 하위 에이전트에게 위임하는 재귀적 구조는 시스템의 일관성과 확장성을 보장합니다.
        

**3. [2단계] 핵심 임무 실행 (Core Mission Execution)**

- **트리거:** 선행 하위 임무들이 성공적으로 완료되었거나, 처음부터 존재하지 않았을 경우.
    
- **오케스트레이터의 행동:**
    
    1. **실행 위임:** 핵심 task_id를 **익스큐터에게 위임**합니다.
        
- **익스큐터의 행동:**
    
    1. 이번에는 tool_type이 없으므로, 하위 에이전트에게 위임하지 않고 **직접 Task를 수행**합니다.
        
    2. tasks.md의 task_purpose와 related_references를 읽고 **플래너가 선택한 방법론을 충실히 이행하여** output_path에 결과물 파일을 생성합니다.
        
- **오케스트레이터의 행동 (성공 시):**
    
    1. 생성된 핵심 결과물 정보를 **전역 db/knowledge_base_catalog.md에 등재**합니다.
        

**4. [3단계] 후행 하위 임무 위임 (Post-Mission Delegation)**

- **트리거:** 핵심 임무가 성공적으로 완료되고, 로드된 Task 명세에 post_tool_purpose가 존재하는 경우.
    
- **오케스트레이터의 행동:**
    
    1. **2단계와 동일한 방식**으로, timing이 POST인 모든 하위 임무를 익스큐터를 통해 순차적으로 위임하고, 그 결과를 처리합니다.
        
    
    - **[사고 모델: 프랙탈(Fractal) 구조]** 핵심 임무 전후에 붙는 하위 임무들은, 전체 워크플로우를 축소해 놓은 작은 프랙탈과 같습니다. 이 구조 덕분에 복잡한 작업도 예측 가능한 패턴으로 처리됩니다.
        

**5. [완료] Task 패키지 상태 최종 확정 (Finalize Task Package Status)**

- **트리거:** 위 모든 단계가 성공적으로 완료된 시점.
    
- **오케스트레이터의 행동:**
    
    1. runs/{run_id}/db/tasks.md에서 현재 task_id의 status를 PENDING에서 COMPLETED로 최종 변경합니다.
        
    2. **전역 db/process_runs.md**의 current_task_id 컬럼을 비워서, 현재 진행 중인 Task가 없음을 명시합니다.
        
    3. 핵심 제어 루프(2.2.2.3.c)로 제어권을 반환하여 다음 PENDING Task를 찾도록 합니다.

---
#### **2.2.4. 오류 처리 (빠른 중단 및 보고)**
...
- **트리거:** **2.2.3. Task 패키지 실행** 프로토콜의 어느 단계에서든 익스큐터가 '성공'이 아닌 '실패' 신호(오류 로그 포함)를 오케스트레이터에게 반환하는 모든 경우.
    
- **주요 행위자:**
    
    - **오케스트레이터:** 오류 발생을 감지하고, 모든 관련 상태를 FAILED로 변경하며, Run 전체를 중단시키는 최종 결정을 내림.
...
익스큐터로부터 '실패' 신호를 감지한 즉시, 오케스트레이터는 다음의 절차를 **다른 모든 작업을 중단하고 최우선으로, 그리고 원자적으로 수행**합니다.

**1. [식별] 실패 지점 특정 (Identify Point of Failure)**

- **오케스트레이터의 행동:**
    
    1. 익스큐터로부터 전달받은 실패 보고에서 **오류가 발생한 정확한 ID (task_id 또는 tool_task_id)** 와 **오류 로그(Error Log)** 를 확보합니다.
        
    2. **전역 db/process_runs.md**를 읽어, 현재 실패가 발생한 run_id, current_phase_id, current_stage_id를 명확히 인지합니다.
...
- **오케스트레이터의 행동:**
    ...
    1. **가장 구체적인 단위부터:**
        ...
        - 만약 실패가 도구 Task에서 발생했다면, runs/{run_id}/db/tool_tasks.md에서 해당 tool_task_id의 status를 FAILED로 변경합니다.
            
        - 실패한 task_id (또는 도구 Task의 parent_task_id)를 찾아, runs/{run_id}/db/tasks.md에서 해당 task_id의 status를 FAILED로 변경합니다.
            
    2. **상위 단위로 전파:**
        
        - runs/{run_id}/db/major_stages.md에서 현재 current_stage_id의 status를 FAILED로 변경합니다.
            
        - runs/{run_id}/db/phases.md에서 현재 current_phase_id의 status를 FAILED로 변경합니다.
            
    3. **최상위 Run 상태 확정:**
        
        - **전역 db/process_runs.md**에서 현재 run_id의 status를 **FAILED로 최종 변경**합니다.
            
- **[아키텍처 원칙: 상태의 일관성]** 실패는 단일 Task의 실패가 아니라, 해당 Task를 포함하는 Stage, Phase, 그리고 Run 전체의 실패입니다. 모든 계층의 상태를 FAILED로 명확히 기록함으로써, 나중에 "어떤 Run이 어디서 실패했는가"를 한눈에 파악할 수 있게 합니다.
...
- **오케스트레이터의 행동:**
    ...
    1. **Communicator 에이전트를 호출**합니다.
        ...
        > **명령:** "Communicator, Run [run_id]가 실패했다. 사용자에게 다음 정보를 명확히 전달하고 모든 작업을 종료하라."
        
    2. Communicator에게 전달할 **실패 보고 패키지**를 구성합니다. 이 패키지에는 다음 정보가 포함됩니다.
        
        - 실패한 run_id
            
        - 실패가 발생한 정확한 지점 (current_phase_id, current_stage_id, 실패한 task_id 또는 tool_task_id)
            
        - 실패한 Task의 목적 (task_purpose)
            
        - 익스큐터가 반환한 원본 **오류 로그 (Error Log)**
...
- **Communicator의 행동:**
    ...
    1. 오케스트레이터로부터 받은 실패 보고 패키지를 바탕으로, 인간이 이해하기 쉬운 형태로 실패 메시지를 구성하여 사용자에게 제시합니다.
        ...
        ```
        [오류] 작업(Run: run-20231028-103000-001) 실행에 실패했습니다.
		
		- 실패 단계: Phase 'ANALYZING' > Stage 'S01'
		- 실패 Task: T02 (경쟁사 B 자료 분석)
		- 원인: [익스큐터가 반환한 오류 로그 원문]
		         예: "FileNotFoundError: 'assets/competitor_B_data.csv' 파일을 찾을 수 없습니다."
		
		작업을 중단합니다. 입력 파일을 확인 후 다시 시도해 주십시오.
		```
...
- **오케스트레이터의 행동:**
    ...
    1. Communicator에게 보고 명령을 전달한 후, 해당 run_id에 대한 **핵심 제어 루프를 완전히 종료**합니다.
        
    2. 더 이상 해당 run_id에 대해 어떠한 계획 수립이나 실행 위임도 시도하지 않습니다. 시스템은 다음 사용자 입력을 기다리는 유휴 상태로 돌아갑니다.
...
- **목적:** 핵심 Task를 보조하는 종속적인 하위 작업을 관리하여 워크플로우의 투명성을 높임.
...
| ---------------------------- | ----------- | ----------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| **Web Search Agent**         | **선행 에이전트** | 주어진 주제에 대한 최신 정보를 웹에서 탐색하고, 결과를 요약하여 workspace/에 보고서 파일로 생성합니다.                                             | Task 실행 전, knowledge_base_catalog에 정보가 부족하거나 최신 정보가 필요할 때. |
| **Data Visualization Agent** | **후행 에이전트** | 주어진 데이터나 분석 결과를 가장 효과적으로 표현할 수 있는 차트/그래프 유형을 **자체적으로 결정**하고, 시각화 자료(HTML)를 생성하여 workspace/에 저장합니다.          | Task 결과물이 복잡한 데이터나 분석 결과를 포함하여 시각적 설명이 필요할 때.              |
| **Image Generation Agent**   | **후행 에이전트** | 주어진 개념이나 텍스트 설명을 바탕으로, 가장 적절한 시각적 은유를 담은 이미지를 생성하기 위한 **상세 프롬프트를 자체적으로 설계**하고, 이미지를 생성하여 workspace/에 저장합니다. | Task 결과물에 대한 추상적인 개념 설명이나 시각적 보조 자료가 필요할 때.                |
