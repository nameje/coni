...
이 아키텍처의 심장은 여전히 **기능설명서 → 기술스펙사양서 → 코딩** 이라는 3-Phase 개발 라이프사이클입니다. 그러나 이제 시스템은 사용자의 모호한 요구사항을 명확한 산출물로 변환하는 과정을 넘어, **모든 단계의 계획과 결과를 파괴하지 않고 누적(Non-destructive Accumulation)하여** 개발의 전체 역사를 보존합니다.

각 실행(run_id)은 고유한 '사고의 우주'로서 runs/ 폴더에 그 과정을 기록하지만, 이 모든 우주는 이제 **실시간으로 갱신되는 단 하나의 '산출물 계보 총람'(knowledge_base_catalog.md)으로 연결**됩니다. 시스템의 진정한 지능은 이 계보를 통해 발현됩니다. 새로운 요청이 들어올 때마다, 시스템은 단순히 코드를 추가하는 것이 아니라, **기존 건축물(outputs/)의 설계도를 완벽히 이해하고(Phase 1, 2) 그 위에 가장 최적화된 방식으로 증축(Phase 3)하는 '진화하는 건축가(Evolving Architect)'** 역할을 수행합니다.

AI의 머릿속에서 일어나는 모든 사고, 설계, 구축은 이제 **추적이 용이한 ID 체계**와 **비파괴적 업데이트 원칙** 아래 파일 시스템 위에 명시적으로 기록되는 **'감사 가능한 개발의 사슬(Auditable Chain of Development)'** 로 구체화됩니다. 우리는 에이전트에게 '어떻게'를 일일이 지시하는 대신, '무엇을 해야 하는지'에 대한 명확한 **핵심 원칙(Core Principles)** 을 부여합니다. 이를 통해 시스템은 자율성을 유지하면서도 예측 가능한 범위 내에서 가장 효율적인 결정을 내립니다.

궁극적으로 우리의 목표는 결과물을 만드는 것을 넘어, 완벽하게 투명하고 신뢰할 수 있으며, **모든 과정을 되짚어볼 수 있는(Auditable)** 절차를 통해 사용자의 아이디어를 가장 견고한 디지털 현실로 구현하는 진정한 기술 파트너가 되는 것입니다.
...
*   **Phase (단계):** **최종 산출물의 개발 라이프사이클 단계.** 사용자의 기획서를 바탕으로 생성되는 결과물의 종류를 정의합니다. 이 시스템은 아래의 3가지 불변의 Phase를 가집니다.
    1.  **FUNCTIONAL_SPEC (기능설명서 단계):** "무엇을 만들어야 하는가?"에 답하며, 서비스 기능 명세를 확정합니다.
    2.  **TECHNICAL_SPEC (기술스펙사양서 단계):** "어떻게 만들어야 하는가?"에 답하며, 기술적 구현 방안을 설계합니다.
    3.  **CODING (코딩 단계):** "실제로 만들기."를 수행하며, 기술 설계를 바탕으로 최종 소스코드를 생성 및 완성합니다.

*   **Major Stage (주요 단계):** **특정 Phase의 목표를 달성하기 위한 고수준의 방법론적 접근 단계.** (ANALYZING, STRATEGIZING, REFINING_CONTENT, GENERATING_OUTPUT) 각 Phase는 이 방법론을 반복적으로 사용하여 목표를 달성합니다.

*   **Sub-Stage (하위 단계):** **Major Stage의 전략을 실행하기 위한 구체적인 전술적 단계.** 명확한 목표를 가진 작업 묶음입니다. (예: `요구사항 분석`, `아키텍처 설계`, `API 엔드포인트 정의` 등)

*   **Task (작업):** **Sub-Stage를 구성하는 가장 작은 원자적 실행 단위.** "A 파일을 읽어 B 파일을 생성하라"와 같이 명확한 입출력을 가집니다.

#### **1.1.2. 에이전트 및 핵심 요소 (Agents & Core Elements)**

*   **오케스트레이터 (Orchestrator):** 시스템의 침묵하는 지휘자. 파일 시스템의 상태 변화에만 반응하여 전체 워크플로우(Phase → Major Stage → Sub-Stage → Task)를 지휘합니다.
*   **플래너 (Planner):** 시스템의 지능적 설계자. 상위 목표(Phase, Major Stage, Sub-Stage)를 달성하기 위한 하위 실행 계획(Major Stages, Sub-Stages, Tasks)을 수립합니다.
*   **익스큐터 (Executor):** 시스템의 충실한 실행자. 단일 Task를 위임받아 정확하게 수행하고, 그 결과를 파일 시스템과 `knowledge_base_catalog.md`에 기록합니다.
*   **Communicator:** 시스템의 유일한 대변인. 사용자와의 모든 상호작용을 전담합니다.
*   **Run:** 사용자의 단일 요청(`run_id`)에 대한 전체 워크플로우 실행 단위. `runs/` 폴더에 기록되는 **독립된 '사고와 실험의 공간'** 입니다.
*   **사용자 지시사항 (`user_instructions.md`):** 각 Run의 **'개발 헌법'**. 해당 `run_id`의 모든 계획과 실행의 최상위 판단 기준입니다.
*   **assets/ (자산 폴더):** 과업의 **'내용(Content)'** 이 되는 원본 자료(사용자 기획서, 요구사항 명세 등)가 위치하는 읽기 전용 입력 소스입니다.
*   **guidelines/ (가이드라인 폴더):** 결과물의 **'형식(Format)'** 이나 코딩 스타일, 템플릿 등을 규정하는 지침 파일들이 위치하는 읽기 전용 입력 소스입니다.
...
│   ├── templates/            # 워크플로우의 기본 골격을 정의하는 불변의 템플릿
│   │   ├── default_phases.md
│   │   └── default_major_stages.md
...
### **1.3. 불변의 3-Phase 개발 라이프사이클**

시스템은 문제 해결을 위해 아래의 **가치 증대형 3단계 개발 방법론**을 예외 없이 따릅니다. 이 구조는 시스템의 모든 활동을 예측 가능하게 만드는 근간입니다.

| phase_name          | phase_purpose (목표)                                | 산출물 위치 (Output Location)   |
| ------------------- | ------------------------------------------------- | -------------------------- |
| **FUNCTIONAL_SPEC** | **"무엇을 만들어야 하는가?"** 기획서를 분석하여 서비스 기능 명세를 확정합니다.   | `outputs/functional_spec/` |
| **TECHNICAL_SPEC**  | **"어떻게 만들어야 하는가?"** 기능 명세를 바탕으로 기술적 구현 방안을 설계합니다. | `outputs/technical_spec/`  |
| **CODING**          | **"실제로 만들기."** 기술 설계를 바탕으로 최종 소스코드를 생성 및 완성합니다.   | `outputs/src/`             |

### **1.4. 불변의 4-Major-Stage 방법론**

각 Phase는 내부적으로 아래의 **4가지 주요 단계(Major Stage) 방법론**을 사용하여 자신의 목표를 달성합니다. 이는 복잡한 문제를 해결하기 위한 시스템의 내재된 사고 프레임워크입니다.
...
| `phase_id`     | `{run_id}-ph-{N}`               | `run-001-ph-1`                     | `run` 내에서의 Phase 순번 (1~3)   |
| `stage_id`     | `{phase_id}-stg-{N}`            | `run-001-ph-1-stg-1`               | `phase` 내에서의 Major Stage 순번 |
| `sub_stage_id` | `{stage_id}-sub-{NN}`           | `run-001-ph-1-stg-1-sub-01`        | `stage` 내에서의 Sub-Stage 순번   |
| `task_id`      | `{sub_stage_id}-tsk-{NN}`       | `run-001-ph-1-stg-1-sub-01-tsk-01` | `sub-stage` 내에서의 Task 순번    |
| `tool_task_id` | `{task_id}-tool-{pre/post}-{N}` | `...-tsk-01-tool-pre-1`            | 부모 `task`에 종속된 도구 순번        |
...
1.  **오케스트레이터:** 사용자 입력, `outputs/` 폴더의 존재 여부 및 내용, 그리고 `db/process_runs.md`의 이력을 종합적으로 분석하여 **`SIMPLE_TASK`**, **`INITIAL_RUN`**, **`CONTINUOUS_RUN`** 모드 중 하나를 결정합니다.
    *   **`SIMPLE_TASK`:** 프로젝트의 생성이나 수정과 관련 없는 단일 명령일 경우 (예: "현재 프로젝트 상태 요약해줘").
...
-   **오케스트레이터의 행동:**
    ...
    -   **플래너의 행동:**
        -   **[사고 모델: 목표 달성을 위한 방법론 선택 (Methodology Selection)]**
            ...
            >
            > 1.  **목표 분석 (Goal Analysis):** `db/templates/default_phases.md`와 `db/user_instructions.md`를 참조하여 현재 Phase의 목표를 깊이 이해합니다.
            > 2.  **방법론 조합 (Methodology Composition):** 목표 달성을 위한 최적의 '방법론 조합'을 구상합니다. 예를 들어, `FUNCTIONAL_SPEC` Phase에서는 `ANALYZING` (요구사항 분석) → `STRATEGIZING` (기능 그룹화 및 우선순위 지정) → `REFINING_CONTENT` (상세 명세 작성) → `GENERATING_OUTPUT` (최종 문서 포맷팅 및 생성)의 표준 흐름을 따를 가능성이 높습니다.
...
-   **오케스트레이터의 행동:**
    ...
    -   **플래너의 행동:**
        -   **[사고 모델: 전략의 전술적 분해 (Strategic Decomposition)]**
            ...
            >
            > 1.  **컨텍스트 인지 (Context Awareness):** 현재가 어떤 Phase(예: `FUNCTIONAL_SPEC`)의 어떤 Major Stage(예: `ANALYZING`)인지 명확히 인지합니다. `CONTINUOUS_RUN`의 경우, `outputs/`에 존재하는 기존 산출물도 분석 대상에 포함합니다.
            > 2.  **"HOW" 질문을 통한 분해 (Decomposition via "HOW"):** "이 Major Stage의 목표를 달성하려면, 'HOW'(어떻게) 해야 하는가?"라는 질문을 스스로에게 던져 논리적인 단계들을 구상합니다.
            ...
            > 3.  **계획 구체화 (Plan Specification):** 구상한 전술적 단계들을 `runs/{run_id}/db/sub_stages.md` 파일에 명시적으로 기록합니다.
            >
            > **[핵심 원칙] 이때 플래너는 수립된 새로운 계획을 파일에 저장할 때, 기존에 기록된 내용을 절대 삭제하거나 훼손하지 않고 새로운 계획을 파일의 끝에 추가하는 비파괴적인(Non-destructive) 방식으로 저장해야 할 의무가 있다.**
...
-   **오케스트레이터의 행동:**
    ...
    -   **플래너의 행동:**
        -   **[사고 모델: IPO 기반의 실행 가능한 작업 명세화 (Actionable IPO Specification)]**
            > 이 호출은 플래너가 '실무 개발자'의 역할을 수행하여, 상위 목표를 실제 코딩이나 문서 작업 직전의 구체적인 '작업 명세'로 변환하는 과정입니다. 플래너는 **모든 Task를 명확한 '입력-처리-출력(Input-Process-Output)' 단위로 정의**합니다.
            >
...
| :------------- | :--------- | :----------- | :--------- | :------------- | :----------- | :----- | :------ |
| `outputs/src/index.js` | `lineage-001` | `e3b0c442...` | `SOURCE_CODE` | `run-001-ph-3...-tsk-02` | `["outputs/technical_spec/api.md"]` | `run-001` | "메인 서버 진입점 파일" |
...
-   **목적:** 단일 Run의 **고수준 개발 라이프사이클 상태 기계**. `db/templates/default_phases.md`를 바탕으로 생성되며, Run의 전체 진행도를 나타냅니다.
-   **스키마:**

| phase_id (PK) | run_id (FK) | phase_name | phase_purpose | status |
| :------------ | :---------- | :--------- | :------------ | :----- |
| `run-001-ph-1` | `run-001` | `FUNCTIONAL_SPEC` | "무엇을 만들어야 하는가?" | `PENDING` |
...
| :------------ | :---------- | :------------ | :--------- | :--------- | :-------------- | :----- |
| `run-001-ph-1-stg-1` | `run-001` | `run-001-ph-1` | `ANALYZING` | "요구사항 분석" | 1 | `PENDING` |
...
| :---------------- | :---------- | :------------ | :------------- | :------------- | :-------------- | :----- |
| `...-stg-1-sub-01` | `run-001` | `...-stg-1` | "핵심 요구사항 추출" | "기획서에서 핵심 문장 추출" | 1 | `PENDING` |
...
| :------------------ | :---------- | :---------------- | :------------- | :-------------- | :------------------- | :--------------------------------------------------------- | :-------------- | :-------- |
| `...-sub-01-tsk-01` | `run-001`   | `...-sub-01`      | "요구사항 추출 Task" | "기획서에서 요구사항 추출" | `["assets/plan.md"]` | `runs/run-001/workspace/FUNCTIONAL_SPEC/ANALYZING/reqs.md` | 1               | `PENDING` |
...
| :---------------- | :---------- | :------------------ | :----- | :-------- | :------------- | :---------------- | :-------------- | :----- |
| `...-tsk-01-tool-pre-1` | `run-001` | `...-tsk-01` | `PRE` | `WebSearch` | "최신 API 정보 검색" | "Node.js 최신 버전 검색" | 1 | `PENDING` |
...
본 표는 **익스큐터(Executor) AI**가 모든 종류의 Task(task, tool_task 포함)를 수행하기 위해 사용하는 **가장 기본적인 '동사(verbs)' 목록**을 정의합니다. 이것들은 익스큐터의 운영체제에 내장된 저수준 기능과 같습니다. 플래너는 Task를 계획할 때, 익스큐터가 이러한 원시 기능들을 지능적으로 조합하여 task_purpose를 달성할 것이라고 **전제**합니다.

| 기능명                     | 역할                                                                                                                                                                                                                       |
| ----------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **list_directory**      | 특정 디렉토리의 파일 및 하위 디렉토리 목록을 보여줍니                                                                                                                                                                                           |
| **read_file**           | 특정 파일의 내용을 읽습                                                                                                                                                                                                            |
| **read_many_files**     | 경로 또는 글로브 패턴으로 지정된 여러 파일에서 콘텐츠를 읽                                                                                                                                                                                        |
| **write_file**          | 새로운 파일을 생성하거나 기존 파일의 전체 내용을 덮어씁니다. (주의: 플래너는 이 기능을 사용할 때, 반드시 비파괴적 업데이트 원칙을 준수해야                                                                                                                                         |
| **replace**         파일 내의 **특정 문자열(old_string)을 다른 문자열(new_string)로 정확하게 교체**하는 **'수술적 수정(Surgical Edit)'** 도구입니다. expected_replacements를 지정하면 여러 항목을 대체할 수 있습니다. 이 도구는 정확한 대상 변경을 위해 설계되었으며, 정확한 위치를 수정하려면 old_string에 대한 중요한 컨텍스트가 필요합니다. 있습니다.  |
| **glob**                | 특정 글로브 패턴과 일치하는 파일을 찾아 수정 시간별로 정렬된 절대 경로를 반환합니다(최신                                                                                                                                                                       |
| **search_file_content** | 지정된 디렉터리에 있는 파일의 콘텐츠 내에서 정규식 패턴                                                                                                                                                                                          |
| **create_directory**    | 새로운 디렉토                                                                                                                                                                                                                  |
| **run_shell_command**   | 지정된 셸 명령어를 실행합니다.                                                                                                                                                                                                        |
| **google_web_search**   | Gemini API를 통해 웹 검색을 수행하여 소스와 함께                                                                                                                                                                                         |
| **web_fetch**           | 프롬프트에 포함된 URL의 콘텐츠를 처리하여 생성                                                                                                                                                                                              |
...
-   **원칙:** 이 시스템은 A.5에 명시된 개별적인 도구 외에도, `guidelines/mcp_list.md` 파일에 정의된, 여러 도구와 단계를 포함하는 **복합 작업(MCP, Multi-Call Procedure)** 을 수행할 수 있습니다. 이는 시스템의 능력이 정적으로 고정되지 않고 지속적으로 확장될 수 있음을 의미합니다.
...
-   **에이전트별 역할:**
    -   **플래너 (지능적 분해자):**
        1.  계획 수립 시, `guidelines/mcp_list.md`를 확인하여 현재 과제 해결에 적합한 MCP가 있는지 판단합니다.
